champ.norm <-
function(beta=myLoad$beta, rgSet=myLoad$rgSet, pd=myLoad$pd,mset=myLoad$mset, sampleSheet="sampleSheet.txt", resultsDir=paste(getwd(), "resultsChamp", sep="/"), methValue="B", fromIDAT=T, norm="BMIQ", fromFile = F, betaFile, filter=T, filterXY=T, QCimages=T, plotBMIQ=T)
{
	detectionP<-NA
	rm(detectionP)
	preprocessSWAN<-NA
