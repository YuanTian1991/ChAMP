                      ...
                      resultsDir="./CHAMP_SVDimages/")
{
   ...
   GenPlot <- function(thdens.o,estdens.o,evalues.v){
    ...
    message("[===========================]")
    message("[If the batch effect is not significant, you may want to process champ.MVP() or champ.DMR() or champ.BlockFinder() next, otherwise, you may want to run champ.runCombat() to eliminat batch effect, then rerun champ.SVD() to check corrected result.]\n")
}
