...
champ.ebayGSEA <- function(beta=myNorm, pheno=myLoad$pd$Sample_Group, adjPval=0.05, arraytype="450K")
{
    #library(Hmisc);
    #library(globaltest);
    ...
    gseaWT.m <- do.call(rbind,lapply(PathwayList,function(x) gseaWTfn(termEID.v=x, rankEID.v=rownames(sresGT.m))))
    colnames(gseaWT.m) <- c("nREP","AUC","P")
    ...
    colnames(topGSEAwt.m) <- c("nREP","AUC","P","adjP");
