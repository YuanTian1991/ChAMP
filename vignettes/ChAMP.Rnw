...
'RPMM', 'sva', 'IlluminaHumanMethylation450kmanifest'))
@
...
The newest version of minfi automatically filters out the 65 SNP probes that were included on the chip as internal controls which can useful for identifying sample mixups. This option is implemented in other packages including \Rpackage{watermelon}. As such, before filtering for low quality probes the dataset will include 485,512 probes.
...
<<lassoFunction, eval=F>>=
lasso=champ.lasso(fromFile=T, limma=limma, image=F,bedFile=F)
if(!is.null(lasso))
{
head(lasso)
}
