...
> We noticed recently champ.load() might not be working on R 3.4, or latest minfi verion. Based on our check, the reason is minfi's updating seems influenced other package, say wateRmelon, champ.load() used to use wateRmelon to get bead count information to do filtering, but now seems not working. **We provided another loading method "ChAMP" for you to load data. The result should be the same as minfi's result and filterings would also be done by champ.filter() function.** Meanwhile, for users want tp use minfi loading function (old ChAMP mode). You may assign "method" parameter in champ.load() as "minfi" to use old ways. Based on our test, on 450K situation, "ChAMP" method and "minfi" method returns EXACTLY the same result, but on EPIC array, since "ChAMP" is now using latest B4 manifest, the results between minfi and ChAMP method would be slightly different.

> **Note that you MUST install [latest ChAMPdata package(version 2.8.1)](https://bioconductor.org/packages/release/data/experiment/html/ChAMPdata.html) to make new ChAMP works.**

1: **Updated EPIC annotation to B4 version:** The B4 version is downloaded from [illumina website](https://support.illumina.com/array/array_kits/infinium-methylationepic-beadchip-kit/downloads.html).

2: **Provided ChAMP method for loading data: **Added one new parameter "method" in champ.load() function, which allows user to choose which method they want to use to read data. ChAMP or Minfi. More detail can be found on Manual of champ.load()

3: **champ.filter() has been totally recoded:** Now users can do any filtering on any data set they want. Merely champ.filter() is focused to take champ.import() result as input and        generate filtered beta value for future analysis. In theory, champ.import() + champ.filter()'s result should be EXACTLY the same as old loading function (minfi method), but since now ChAMP is using B4 verion for EPIC array, the two method have slightly difference on final result, but merely the same. For 450K array, ChAMP method and minfi method are exactly the same.

4: **Provide Whole New function champ.import() to read IDAT file to R:**, which is similar to minfi's read.meth.exp() function, but will NOT return rgSet or Mset object. Also remember that "ChAMP" method's result can not be run in SWAN and FunctionalNormliazation method. If you want to use them, please set "metehod" parameter in champ.load() as "minfi" and use minfi method.

5: **Conflict Checking in champ.runCombat():** Added more strict checking in champ.runCombat(), now champ.runCombat() would check if your variable and batches conflict with each other. When it comes to "conflict", it means one group of your variable (default is Sample_Name), is totally belongs to only one batches. Combat() function from SVA can not do batch correction on this situation.

6: **Added Demo on github**: In respond to our reviewer's question and to make users have better understanding on our package, we processed ChAMP fully on some data sets and saved all messages shown during processing. We upload these information to [github](https://github.com/JoshuaTian/ChAMPDemos).

7: **Added Hydroxymethylation in Vignette:** ChAMP can also be used to analysis hydroxymethylation data, we provided a small demo part in vignette to show user how to use it.
...
For users who need to find Differentially Methylated Blocks [@Hansen2011]. New version ChAMP includes a function to detect methylation Blocks. Another important function is champ.GSEA(), which would take singnificant genes related with DMPs and DMRs and do GSEA analysis on them, to see what pathways are enriched by significant CpGs or DMRs. To correct biased might be caused by ininequality between genes and CpG probes, we used goseq pacakge to correct the bias [@Young2010]. Also, new version ChAMP incoporates FEM package [@Jiao15082014], which would calculate modules in pathways that show differential methylation changes between two phenotypes.
...
Loading Data is the first step for all analysis. ChAMP provides a loading function to get data from .idat file (with pd file (Sample_Sheet.csv) in it). The old champ.load() function utilises minfi to load data from .idat files, but now we provided a neww method "ChAMP" to read data. They would return merely the same result except minfi would also return rgSet and mset, which might be used in SWAN and FunctionNormliazation method in champ.norm(). By default this loads data from the current working directory, in this directory you should have all .idat files and a sample sheet. The sample sheet currently needs to be a .csv file as came with your results following hybridization. You can choose whether you want M-values or beta-values. For small datasets M-values are recommended [@Zhuang12]. The champ.load() function used to load the data from the idat files would automatically filters out the SNPs that might influent the analyais result. The SNP list are generated from [Zhou's Nucleic Acids Research paper in 2016](http://zwdzwd.github.io/InfiniumAnnotation) [@Zhou2016], which is designed specifically for 450K and EPIC, also with population differences considered. As such, for 450k bead array data, before filtering for low quality probes, the dataset will include 485,512 probes. And for EPIC bead array data, before filtering probes the dataset, will include 867,531 probes.

Currently the default method for loading is "ChAMP". You may set "method" in champ.load() to use classical minfi way. **Note that "ChAMP" method in champ.load() is merely a combination of champ.import() and champ.filter()**. If you are not satisfied with result returned by champ.load(), say you want to get beadcount,detect P matrix, or Meth UnMeth matrix, you may use champ.import() to generate them.
...
## Filtering Data
ChAMP provided a comprehensive filtering function called `champ.filter()` which could take any data matrix (beta, M, Meth, UnMeth, intensity) and do all filtering on them, including imputation, which is done by champ.impute() function. champ.filter() does not call for any specific object or class result, nor need IDAT file, as long as you have a single beta matrix, the filtering can be done. 

Of course for some filtering method, some accesory data need to be provided, say if you want to do filtering on detect P value, at least you need to provide detect P matrix. Als if you want to do filtering on beadcount, at least you need to provide beadcount. 

champ.filter() would check your inpute strictly beforehead, basically there are some rules:

* First, you must input at least on data matrix.
* Second, if you inputted multiple matrix, they must have EXACTLY the same rownames and colnames. For example, if you want to do filtering on intensity data and M value at the same time, you MUST make sure they have the same name, otherwise, ChAMP would suspect they are data comes from different source.
* Third, if you want to filter pd too. (filterDetP process might remove some low quality samples.) Please make sure Sample_Name in your pd file is exactly the same as colnames of your data matrix.
* Fourth, if you want to do imputation, you must provide detect P matrix, beta or M matrix, and parameter ProbeCutoff can not be 0. If any of these three condition fails, champ.filter() would reset autoimpute parameter as FALSE.
* Five, if you want to filter beadcount, you must provie beadcount as well.

Merely champ.filter() is the after process of champ.import() which is a noval loading function coded by ChAMP team. You may use them like below:
```{r eval=FALSE}
...
Actually During the loading process, champ.load() function would also do some filtering. Below are filtering would be done by both champ.load() and champ.filter().
...
* Fourth, ChAMP will filter all SNP-related probes. The SNP list comes from [Zhou's Nucleic Acids Research paper in 2016](http://zwdzwd.github.io/InfiniumAnnotation). Note that if you know which population your data is, you can choose certian population to do filtering. Otherwise ChAMP would use General Recommended Probes provided by Zhou to do filtering.
...
```
After Normalization, you may use QC.GUI() function to check the result again, remember to set beta parameter in QC.GUI() function as myNorm.
...
In the latest version, champ.runCombat() would also check if user's Batch and Variable conflict with each other. Conflict likes all samples from one phenotype of your Sample_Name are comes from certain one Batch, which means, if you correct batches, your Sample_Name-contained information are also gone, which is not allowed in Combat function of SVA package, that's why we made check beforehead.
...
Some users might want to analysis hydroxymethylation data, which can be done with champ.DMP() function, here we provided some demo code below for user to check.
